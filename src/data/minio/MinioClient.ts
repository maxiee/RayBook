...
async function checkAndCreateBucket(bucketName: string) {
  ...
  try {
    ...
    logService.info(`Bucket "${bucketName}" 已经存在。`);
  } catch (error) {
    // 如果 Bucket 不存在，则根据错误类型决定操作
    if (error.statusCode === 404) {
      logService.info(`Bucket "${bucketName}" 不存在，正在尝试创建...`);
      try {
        await s3Client.createBucket(params).promise();
        logService.info(`Bucket "${bucketName}" 创建成功。`);
      } catch (createError) {
        logService.error("创建 Bucket 失败：", createError);
      }
    } else {
      logService.error("检查 Bucket 时发生错误：", error);
    }
  }
}
