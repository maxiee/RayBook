...
// 获取书籍详情
ipcMain.handle('get-book-details', async (event, bookId: string): Promise<IBook | null> => {
  try {
    const book = await bookRepository.findBookById(bookId);
    return DtoUtils.toTransferable(book);
  } catch (error) {
...
ipcMain.handle('update-book', async (event, book: IBook | null) => {
  try {
    const updatedBook = await bookRepository.updateBook(book);
    return DtoUtils.toTransferable(updatedBook);
  } catch (error) {
...
ipcMain.handle('upload-book-file', async (event, bookId: string): Promise<IBookFile | null> => {
  try {
    ...
    return DtoUtils.toTransferable(newUploadBookFile);
  } catch (error) {
...
ipcMain.handle('add-book', async (event, book: IBook): Promise<IBook | null> => {
  try {
    ...
    // 保存到数据库
    await newBook.save();
    return DtoUtils.toTransferable(newBook);
  } catch (error) {
