...
// 上传图书文件
ipcMain.handle('upload-book-file', async (event, bookId: Id): Promise<IBookFile | null> => {
  try {
...
ipcMain.handle('add-new-book', async (event) => {
  ...
  const coverIamgeUrl = await coverImageRepository.uploadBookCoverImage(newBook._id, filePath);
